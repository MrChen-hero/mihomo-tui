...
export function DelayBadge({ status, delay, testing, width = 7 }: DelayBadgeProps) {
  if (testing) {
    return (
      <Text color="cyan">{'测速中'.padStart(width)}</Text>
    )
  }
  switch (status) {
    case 'good':
      return <Text color="green">{`${delay}ms`.padStart(width)}</Text>
    case 'fair':
      return <Text color="yellow">{`${delay}ms`.padStart(width)}</Text>
    case 'slow':
      return <Text color="red">{`${delay}ms`.padStart(width)}</Text>
    case 'dead':
      return <Text color="red">{'超时'.padStart(width - 2)}</Text>
    default:
